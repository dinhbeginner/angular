...
<div class="row">
<ng-container *ngFor="let item of displayedProductList | paginate: {itemsPerPage:12,currentPage:p}">
    <div class="col-lg-3 col-md-3 col-sm-4 col-6">
      <div class="card-deck mt-3">
        <div class="card">
          <img class="card-img-top p-3" [src]="item.imageUrl" alt="{{ item.productName }}" width="100%" height="50%">
          <div class="card-body text-center">
            <h4 class="card-title">
              <a [routerLink]="['/product-details', item.id]">{{ item.productName }}</a>
            </h4>
            <p class="card-text">{{ item.price | currency:'USD':true }}</p>
            <button type="button" class="btn btn-primary" [routerLink]="['/product-details', item.id]">See Details</button>
          </div>
        </div>
      </div>
    </div>
